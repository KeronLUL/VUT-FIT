...
*/
	...
	// Set every value to '*' and set indexes to 0
	if (q != NULL) {
...
*/
	...
	// Return value depending if the queue is empty
...
*/
	...
	// Return value depending if the queue is full
...
*/
	...
	// If queue isn't empty return char from the front
	if (!queueEmpty(q)) {
		*c = q->arr[q->f_index];
	}
...
*/
	...
	// If queue isn't empty remove char from the front
	if (!queueEmpty(q)) {
...
*/	
	...
	// If queue isn't empty remove char from the front and return it through parametr c
	if (!queueEmpty(q)) {
		queueFront(q, c);
		queueRemove(q);
	}
...
*/	
	...
	// Id queue isn't full insert char c into the queue
	if (!queueFull(q)) {
		q->arr[q->b_index] = c;
